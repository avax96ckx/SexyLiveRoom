...
function AuthCallback() {
  ...
  const { login: authLogin } = useAuth() as any;
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        if (error) { console.error('[AuthCallback] Error:', error); navigate('/login'); return; }
        if (data.session?.user) {
          const u = data.session.user;
          const { data: prof } = await supabase.from('user_profiles').select('*').eq('id', u.id).single();
          if (!prof) {
            await supabase.from('user_profiles').upsert({
              id: u.id,
              username: u.user_metadata?.full_name || u.user_metadata?.name || u.email?.split('@')[0] || 'User',
              email: u.email || '',
              google_email: u.email || '',
              balance: 0, referral_count: 0, phone_visible: false,
              is_vip: false, is_business: false, is_blocked: false, account_status: 'active',
              referral_code: Math.random().toString(36).substring(2, 10).toUpperCase(),
              avatar_url: u.user_metadata?.avatar_url || u.user_metadata?.picture || null,
            });
          }
          const { data: finalProf } = await supabase.from('user_profiles').select('*').eq('id', u.id).single();
          authLogin(u, finalProf);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#0a030f' }}>
      <div className="text-center">
        <div className="text-4xl mb-4">💋</div>
        <div className="w-10 h-10 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-3" />
        <p className="text-white font-semibold">Inaingia...</p>
      </div>
    </div>
  );
}
