...
export default function Admin() {
  ...
  const [bulkHistory, setBulkHistory] = useState<Array<{time: string; count: number; amount: number}>>(() => {
    try { return JSON.parse(localStorage.getItem('slr_bulk_balance_history') || '[]'); } catch { return []; }
  });
        ...
        {tab === 'seo' && (
          <div className="mt-4 space-y-4" onFocus={markEditing} onChange={markEditing} onInput={markEditing}>
            ...
            <button onClick={async () => {
              ...
              // Sync to settings state and dispatch update event
              setSettings(prev => ({ ...prev, ...Object.fromEntries(seoKeys.map(k => [k, editSettings[k] || ''])) }));
              window.dispatchEvent(new CustomEvent('app-settings-updated', { detail: Object.fromEntries(seoKeys.map(k => [k, editSettings[k] || ''])) }));
              try { localStorage.setItem('slr_settings_cache', JSON.stringify({ ...settings, ...Object.fromEntries(seoKeys.map(k => [k, editSettings[k] || ''])) })); } catch {}
              toast.success('✅ SEO imehifadhiwa! Picha mpya ya OG itaonekana mara moja ukishare link.');
              setSaving(false);
            ...
            <button onClick={async () => {
              setSaving(true);
              try {
                const [{ data: malayaPosts2 }, { data: videoPosts2 }] = await Promise.all([
                  supabase.from('content_posts').select('id,created_at').eq('type', 'malaya').order('created_at', { ascending: false }).limit(500),
                  supabase.from('content_posts').select('id,created_at').eq('type', 'video').order('created_at', { ascending: false }).limit(500),
                ]);
                const base = 'https://sexy-live-room.vercel.app';
                const now = new Date().toISOString().split('T')[0];
                const staticUrls = ['/', '/malaya', '/video', '/tiksexy', '/live', '/sexyroom'];
                const malayaUrls = (malayaPosts2 || []).map((p: any) => `\n  <url>\n    <loc>${base}/malaya?post=${p.id}</loc>\n    <lastmod>${(p.created_at || now).split('T')[0]}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.8</priority>\n  </url>`);
                const videoUrls = (videoPosts2 || []).map((p: any) => `\n  <url>\n    <loc>${base}/video?post=${p.id}</loc>\n    <lastmod>${(p.created_at || now).split('T')[0]}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.8</priority>\n  </url>`);
                const staticXml = staticUrls.map(u => `\n  <url>\n    <loc>${base}${u}</loc>\n    <lastmod>${now}</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>1.0</priority>\n  </url>`).join('');
                const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${staticXml}${malayaUrls.join('')}${videoUrls.join('')}\n</urlset>`;
                const blob = new Blob([xml], { type: 'application/xml' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a'); a.href = url; a.download = 'sitemap.xml'; a.click(); URL.revokeObjectURL(url);
                toast.success(`✅ Sitemap imeundwa! Posts ${(malayaPosts2 || []).length + (videoPosts2 || []).length} zimewekwa. Pakia kwenye public/ folder.`);
              } catch { toast.error('Hitilafu ya kuunda sitemap'); }
              finally { setSaving(false); }
            }} disabled={saving} className="btn-outline w-full flex items-center justify-center gap-2 text-sm">
              {saving ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Inaunda...</> : '🗺️ Tengeneza & Download Sitemap.xml'}
            </button>
          </div>
        )}
          ...
          <div className="space-y-2 mt-2">
            ...
            <div className="content-box p-3 space-y-2">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => setBulkSelectedIds(filteredMembers.map(m => m.id))}
                  className="flex-1 py-2 rounded-xl text-xs font-bold bg-primary/20 text-primary border border-primary/30 active:scale-95"
                >
                  ✅ Chagua Wote ({filteredMembers.length})
                </button>
                <button
                  onClick={() => setBulkSelectedIds([])}
                  disabled={bulkSelectedIds.length === 0}
                  className="flex-1 py-2 rounded-xl text-xs font-bold bg-gray-600/20 text-gray-400 border border-gray-600/30 active:scale-95 disabled:opacity-40"
                >
                  ✖ Futa Uchaguzi
                </button>
              </div>
              {bulkSelectedIds.length > 0 && (
                <p className="text-primary text-xs font-semibold">{bulkSelectedIds.length} member wamechaguliwa - bonyeza tena ili kuwaondoa</p>
              )}
            </div>
              ...
              <div key={m.id} className={`content-box p-3 cursor-pointer transition-all ${bulkSelectedIds.includes(m.id) ? 'border-primary/70 bg-primary/5' : ''}`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                    bulkSelectedIds.includes(m.id) ? 'bg-primary border-primary' : 'border-gray-600 bg-transparent'
                  }`}>
                    {bulkSelectedIds.includes(m.id) && <Check className="w-3 h-3 text-white" />}
                  </div>
