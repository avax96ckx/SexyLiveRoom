...
export default function VideoSection() {
  ...
  const [sharePreviewPost, setSharePreviewPost] = useState<ContentPost | null>(null);
  // X ZA ADMIN code
  ...
  function sharePostWhatsApp(post: ContentPost, e?: React.MouseEvent) {
    e?.stopPropagation();
    const shareUrl = `https://sexy-live-room.vercel.app/video?post=${post.id}`;
    const shareText = `Tazama video hii sasa kwenye Sexy Live Room 🎬\n${shareUrl}`;
  ...
  async function sharePost(post: ContentPost, e?: React.MouseEvent) {
    e?.stopPropagation();
    const shareUrl = `https://sexy-live-room.vercel.app/video?post=${post.id}`;
    try { if (navigator.share) await navigator.share({ title: post.title || 'Video', url: shareUrl }); else { await navigator.clipboard.writeText(shareUrl); toast.success('Link imenakiliwa!'); } }
    catch { await navigator.clipboard.writeText(shareUrl).catch(() => {}); toast.success('Link imenakiliwa!'); }
  }
  ...
  return (
    <div className="min-h-screen bg-background">
                  ...
                  return (
                    <div key={post.id} ref={el => { postRefs.current[post.id] = el; }}
                      ...
                      <div className="flex items-center gap-2 px-4 py-2">
                        ...
                        <button onClick={e => sharePostWhatsApp(post, e)} className="ml-1 w-7 h-7 flex items-center justify-center rounded-lg text-green-400" style={{ background: 'rgba(37,211,102,0.15)', border: '1px solid rgba(37,211,102,0.25)' }}>
                        ...
                        <button onClick={e => { e.stopPropagation(); setSharePreviewPost(post); }} className="text-gray-400 ml-1"><Share2 className="w-4 h-4" /></button>
      ...
      {/* Share Preview Modal */}
      {sharePreviewPost && (() => {
        const post = sharePreviewPost;
        const mediaUrls: string[] = (post.media_urls && post.media_urls.length > 0) ? post.media_urls : (post.media_url ? [post.media_url] : []);
        const thumbUrl = post.thumbnail_url || (post as any).thumb_urls?.[0] || '';
        const shareUrl = `https://sexy-live-room.vercel.app/video?post=${post.id}`;
        const shareText = `Tazama video hii sasa kwenye Sexy Live Room 🎬`;
        return (
          <div className="fixed inset-0 z-50 bg-black/90 flex items-end justify-center" onClick={() => setSharePreviewPost(null)}>
            <div className="w-full max-w-md bg-[#0d0d0d] border border-[#3d0b3d] rounded-t-3xl p-5 slide-up" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between mb-4">
                <h3 className="text-white font-bold">👁️ Preview ya Share</h3>
                <button onClick={() => setSharePreviewPost(null)}><X className="w-5 h-5 text-gray-400" /></button>
              </div>
              <div className="rounded-2xl overflow-hidden border border-white/10 mb-4" style={{ background: 'rgba(30,10,40,0.9)' }}>
                {thumbUrl ? (
                  <img src={thumbUrl} alt="" className="w-full" style={{ height: '200px', objectFit: 'cover', objectPosition: 'top' }} />
                ) : (
                  <div className="w-full flex items-center justify-center" style={{ height: '200px', background: '#1a0a1a' }}>
                    <span className="text-5xl">🎬</span>
                  </div>
                )}
                <div className="p-3">
                  <p className="text-[10px] text-gray-500 uppercase mb-1">sexy-live-room.vercel.app</p>
                  <p className="text-white font-bold text-sm">{post.title ? `🎬 ${post.title} - SEXY LIVE ROOM` : '🎬 Video - SEXY LIVE ROOM'}</p>
                  <p className="text-gray-400 text-xs mt-1 line-clamp-2">{shareText}</p>
                </div>
              </div>
              <p className="text-gray-500 text-xs text-center mb-4">Hivi ndivyo itakavyoonekana ukishare kwenye WhatsApp, Facebook</p>
              <div className="flex gap-3">
                <button onClick={() => { sharePostWhatsApp(post); setSharePreviewPost(null); }}
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-black text-white text-sm"
                  style={{ background: 'linear-gradient(135deg, #25d366, #128c7e)' }}>
                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  WhatsApp
                </button>
                <button onClick={async () => { await navigator.clipboard.writeText(`${shareText}\n${shareUrl}`).catch(() => {}); toast.success('Link imenakiliwa!'); setSharePreviewPost(null); }}
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-white text-sm bg-[#1a0a1a] border border-white/20">
                  📋 Nakili Link
                </button>
              </div>
            </div>
