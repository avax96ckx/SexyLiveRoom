...
export default function MalayaSection() {
  ...
  function sharePostWhatsApp(post: ContentPost, e?: React.MouseEvent) {
    e?.stopPropagation();
    const shareUrl = `https://sexy-live-room.vercel.app/malaya?post=${post.id}`;
    const shareText = `Chukuwa bure namba zake kwenye Sexy live room bonyeza hapa chini kwenye link👇\n${shareUrl}`;
  ...
  return (
    <div className="min-h-screen bg-background pb-4">
              ...
              return (
                <div key={post.id} ref={el => { postRefs.current[post.id] = el; }} className={`border-b border-[#1a0a1a] transition-all duration-300 ${highlightPostId === post.id ? 'bg-primary/10' : ''} ${post.is_boosted && post.boost_expires_at && new Date(post.boost_expires_at) > new Date() ? 'border-l-2 border-l-orange-500' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 px-4 py-2">
                    ...
                    <button onClick={e => { e.stopPropagation(); sharePostWhatsApp(post); }} className="w-7 h-7 flex items-center justify-center rounded-lg text-green-400" style={{ background: 'rgba(37,211,102,0.15)', border: '1px solid rgba(37,211,102,0.25)' }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); setSharePreviewPost(post); }} className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#1a0a1a] text-gray-400"><Share2 className="w-3.5 h-3.5" /></button>
      ...
      {sharePreviewPost && (() => {
        ...
        const mediaUrls: string[] = (post.media_urls && post.media_urls.length > 0) ? post.media_urls : (post.media_url ? [post.media_url] : []);
        const isVid = mediaUrls[0] && /\.(mp4|webm|mov)/i.test(mediaUrls[0]);
        const thumbUrl = (post as any).thumb_urls?.[0] || post.thumbnail_url || (!isVid ? mediaUrls[0] : '');
        const shareUrl = `https://sexy-live-room.vercel.app/malaya?post=${post.id}`;
        const shareText = `Chukuwa bure namba zake kwenye Sexy live room bonyeza hapa chini kwenye link👇`;
        return (
            ...
            <div className="w-full max-w-md bg-[#0d0d0d] border border-[#3d0b3d] rounded-t-3xl p-5 slide-up" onClick={e => e.stopPropagation()}>
              ...
              <div className="rounded-2xl overflow-hidden border border-white/10 mb-4" style={{ background: 'rgba(30,10,40,0.9)' }}>
                ...
                <div className="p-3">
                  ...
                  <p className="text-white font-bold text-sm">{post.title || 'SEXY LIVE ROOM - Malaya'}</p>
                  <p className="text-gray-400 text-xs mt-1 line-clamp-2">{shareText}</p>
                </div>
              </div>
              <p className="text-gray-500 text-xs text-center mb-4">Hivi ndivyo itakavyoonekana ukishare kwenye WhatsApp, Facebook na mitandao mingine</p>
              <div className="flex gap-3">
