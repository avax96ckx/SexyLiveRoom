...
// ─── LocalStorage Message Cache (survives navigation) ───────────────────────
const MSG_CACHE_KEY = 'slr_msg_cache_v2';
const MSG_TTL = 5 * 60 * 1000; // 5 minutes

function addMsgToCache(msg: any) {
  try {
    const raw = localStorage.getItem(MSG_CACHE_KEY);
    const cache: any[] = raw ? JSON.parse(raw) : [];
    const now = Date.now();
    const fresh = cache.filter(m => now - new Date(m.created_at).getTime() < MSG_TTL);
    // Avoid duplicates
    const exists = fresh.some(m => m.id === msg.id);
    if (!exists) fresh.push(msg);
    localStorage.setItem(MSG_CACHE_KEY, JSON.stringify(fresh.slice(-100)));
  } catch {}
}

function loadMsgCache(): any[] {
  try {
    const raw = localStorage.getItem(MSG_CACHE_KEY);
    if (!raw) return [];
    const cache: any[] = JSON.parse(raw);
    const now = Date.now();
    return cache.filter(m => now - new Date(m.created_at).getTime() < MSG_TTL);
  } catch { return []; }
}

function cleanMsgCache(realMsgs: any[]) {
  try {
    const raw = localStorage.getItem(MSG_CACHE_KEY);
    if (!raw) return;
    const cache: any[] = JSON.parse(raw);
    // Remove any cached msg that now exists in real DB results
    const cleaned = cache.filter(opt => !realMsgs.some(real =>
    ...
    localStorage.setItem(MSG_CACHE_KEY, JSON.stringify(cleaned));
  } catch {}
}
...
export default function SexyRoom() {
  ...
  // Load cached messages immediately so they survive navigation
  const [messages, setMessages] = useState<RoomMessage[]>(() => {
    const cached = loadMsgCache();
  ...
  async function fetchMessages(scrollToBottom = true) {
    ...
    // Merge: keep optimistic messages that don't yet have a real match
    // Clean cache of messages now confirmed in DB
    cleanMsgCache(msgs);
    // Merge: real DB messages + any still-pending cached messages
    const cachedPending = loadMsgCache().filter(opt =>
      !msgs.some(real =>
        real.user_id === opt.user_id &&
        real.content === opt.content &&
        Math.abs(new Date(real.created_at).getTime() - new Date(opt.created_at).getTime()) < 30000
    ...
    setMessages([...msgs, ...cachedPending]);
    if (scrollToBottom) {
  ...
  async function sendMessage() {
    ...
    // Optimistic update: show message immediately + cache it in localStorage (survives navigation)
    if (!hasMedia && hasContent && user && profile) {
      ...
      // Add to localStorage cache so message persists if user navigates away and returns
      addMsgToCache(optimisticMsg);
    ...
    try {
      ...
      if (hasMedia) {
        ...
        if (uploadedUrls.length > 0) {
          ...
          // Optimistic update for media - show immediately before DB confirm
          const optimisticMediaMsg: any = {
          ...
          addMsgToCache(optimisticMediaMsg);
      ...
      setUploadProgress(0); setUploadedMB(0); setTotalUploadMB(0); setViewOnce(false); setUploadSpeedMBs(0);
