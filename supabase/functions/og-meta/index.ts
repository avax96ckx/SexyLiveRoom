...
serve(async (req) => {
  ...
  try {
    ...
    // ─── ALWAYS fetch site settings from DB (no cache) for real-time updates ───
    let siteTitle = 'SEXY LIVE ROOM 💋'
    let siteDescription = 'Platform ya burudani - Malaya, Video, Live na zaidi! Jiunge sasa hivi.'
    let siteOgImage = `${siteOrigin}/og-image.jpg`

    try {
      const settingsResp = await fetch(
        `${supabaseUrl}/rest/v1/app_settings?key=in.(site_title,site_description,site_og_image,site_keywords)&select=key,value`,
        { headers: dbHeaders }
      )
      const settingsData = await settingsResp.json()
      if (Array.isArray(settingsData)) {
        for (const row of settingsData) {
          if (row.key === 'site_title' && row.value) siteTitle = row.value
          if (row.key === 'site_description' && row.value) siteDescription = row.value
          if (row.key === 'site_og_image' && row.value) siteOgImage = row.value
        }
      }
    ...
    if (type === 'post' || type === 'malaya') {
      try {
        const resp = await fetch(
          `${supabaseUrl}/rest/v1/content_posts?id=eq.${id}&select=title,media_url,thumbnail_url,description,location,type,region,media_urls,thumb_urls,created_at`, 
          { headers: dbHeaders }
        )
        ...
        if (post) {
          ...
          // Use thumbnail first (best quality), then first thumb_url, then first media
          const thumbUrls = post.thumb_urls?.length ? post.thumb_urls : []
          image = post.thumbnail_url || thumbUrls[0] || mediaUrls[0] || image
          // If image is a video URL, fall back to og-image
          if (image && /\.(mp4|webm|mov|avi|mkv)/i.test(image)) {
            image = thumbUrls[0] || post.thumbnail_url || siteOgImage
          }
          ogType = 'article'
    ...
    if (type === 'video') {
      try {
        ...
        if (post) {
          ...
          // Prefer thumbnail (image), never use video URL as og:image
          image = post.thumbnail_url || siteOgImage
          if (image && /\.(mp4|webm|mov|avi|mkv)/i.test(image)) image = siteOgImage
          ogType = 'video.other'
    ...
    // Final safety: if image is still a video URL, use default og image
    if (image && /\.(mp4|webm|mov|avi|mkv)/i.test(image)) {
...
<head>
  ...
  <script>
    (function() {
      ...
      var isCrawler = /facebookexternalhit|facebookcatalog|twitterbot|telegrambot|whatsapp|discordbot|linkedinbot|slackbot|googlebot|bingbot|applebot|ia_archiver/i.test(ua);
      if (!isCrawler) {
